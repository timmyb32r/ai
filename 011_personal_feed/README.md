...
Go-сервис → SQLite → 41 независимая RSS-лента для Inoreader. Девять ранее выбранных источников с готовыми RSS/Atom исключены из конфигурации для прямой подписки в Inoreader. Источники: RSS/Atom/JSON Feed, HTML с CSS-селекторами, Chromium для JavaScript и специализированные адаптеры публичных данных сайтов.
...
Добавленные источники: `/airbyte.xml`, `/astronomer.xml`, `/openlineage.xml`, `/meltano.xml`, `/onehouse.xml`, `/slingdata.xml`. Cockroach Labs (`/cockroachlabs.xml`), Databricks (`/databricks.xml`) и DBConvert (`/dbconvert.xml`) уже были в списке, их адреса сохранены. Databricks и DBConvert при последней проверке блокировали загрузку; подробности в [отчёте источников](docs/SOURCES.md).

Из китайского списка в сервисе остаются 11 источников, включая InfoQ, 墨天轮 и Digoal. DTinsight, Greptime 中文, Databend 中文, Vonng и Meituan исключены: пользователь подписался на их RSS напрямую в Inoreader. [Адреса оставшихся лент и история проверки](docs/CHINESE_SOURCES.md). Китайский TapData доступен отдельно по `/tapdata-cn.xml`.
...
`feed_url` используется первым и поддерживает RSS, Atom и JSON Feed. Если в нём меньше `initial_items`, HTML и дополнительные страницы дополняют начальную выборку. При сбое ленты используется HTML. Без явного RSS и без CSS-селектора выполняется поиск RSS/Atom в `<link rel="alternate">`; проверяйте, что это нужный раздел, а не общий поток или releases.

Дополнительные параметры источника:

- `listing_url`: адрес списка, отличающийся от публичного URL блога (например, HTML API IBM).
- `date_selector`: элемент с датой в карточке списка или на странице статьи. Поддерживаются `datetime`, `content` и текст; уже известная дата не заменяется. Например, у Meltano: `.blog-list-item-date, .blog-author-date .bold`.
- `title_selector`: явный заголовок в карточке и на странице статьи; имеет приоритет над общими заголовками сайта и Open Graph. Если элемент отсутствует, используются прежние правила извлечения.
...
- `adapter`: специализированный разбор: `cloudera`, `infoq-bigdata`, `modb-news`, `pingkai`, `mirrorship` или `digoal`. Подробности новых адаптеров и их ограничения — в [отчёте китайских источников](docs/CHINESE_SOURCES.md).
