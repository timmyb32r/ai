...
func (h *HanLPTokenizer) Segment(text string) ([]models.Token, error) {
	return h.SegmentContext(context.Background(), text)
}

func (h *HanLPTokenizer) SegmentContext(ctx context.Context, text string) ([]models.Token, error) {
	if text == "" {
		return nil, nil
	}
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, h.url+"/parse", bytes.NewReader(body))
	if err != nil {
		return nil, err
	}
